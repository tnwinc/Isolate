...
task :build do
  ...
  system("cd #{root} && bundle exec `npm bin`/coffee --compile spec")
end
...
test_namespace = namespace :test do
  ...
  task :commonjs => :build do
    debug = isDebug?() ? ' debug' : ''
    Dir.chdir root do
      system "NODE_PATH=.:./spec:./spec/modules_for_testing/commonjs:$NODE_PATH `npm bin`/mocha --compilers coffee:coffee-script --reporter spec #{debug} ./spec/commonjs.spec.coffee"
    end
  end
  ...
  task :requirejs => :build do
    ...
    versions_to_test_against.each do |version|
      Dir.chdir root do
        ...
        cmd = "NODE_PATH=.:./spec:$NODE_PATH `npm bin`/mocha --compilers coffee:coffee-script --globals 'define,requirejsVars' --reporter spec #{debug} ./spec/requirejs.spec.coffee"
        if isDebug?()
          system cmd
        else
